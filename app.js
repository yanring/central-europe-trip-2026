...
async function copyFavorites(){
  ...
  const lines=["# 我的中欧旅行收藏","",`复制时间：${new Date().toLocaleString("zh-CN",{hour12:false})}`,`共 ${places.length} 个收藏地点；这是候选清单，不代表已经预约。`,""];
  ...
  try{
    ...
    toast(`已复制 ${places.length} 个收藏地点和对应备注，可以直接粘贴。`);
  }catch(e){
    if(modal.open)modal.close();
    editorContext=null;
    modal.innerHTML=`<div class="dialog-body"><div class="dialog-head"><h2>手动复制收藏清单</h2><button type="button" class="small ghost" data-action="close-dialog" aria-label="关闭">×</button></div><p>浏览器未允许自动复制。请复制下方已选中的文字，手机上可长按全选后复制。</p><label class="label" for="favorites-copy-text">收藏清单</label><textarea id="favorites-copy-text" class="edit-timeline" readonly>${esc(content)}</textarea></div>`;
    modal.showModal();
    const field=document.getElementById("favorites-copy-text");
...
document.addEventListener("click",e=>{
  ...
  else if(action==="copy-favorites")copyFavorites();
